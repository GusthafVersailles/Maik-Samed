...
/* =========================================================
   BASE DE PRODUTOS
   - Cada objeto representa um produto do catálogo
   - Campos principais:
     id, name, category, type, color, description, image
     detailDescription, highlights, additionalInfo
     metaTags, brandBadges
   ========================================================= */
const products = [
  {
    id: 1,
...
/* =========================================================
   ...
   - category: categoria selecionada no filtro
   - search: termo digitado na busca
   - cart: array com produtos adicionados ao carrinho
   ========================================================= */
const state = {
  category: 'Todos',
  search: '',
...
/* =========================================================
   ...
   - Centraliza os seletores usados no projeto
   ========================================================= */
const elements = {
...
/* =========================================================
   ...
   - "Todos" + categorias extraídas automaticamente dos produtos
   ========================================================= */
...
/* =========================================================
   ...
   - Marca a categoria ativa com classe "active"
   ========================================================= */
function renderFilters() {
  elements.filterPills.innerHTML = categories
    .map((category) => `
      <button
        class="filter-pill ${state.category === category ? 'active' : ''}"
...
/* =========================================================
   VISUAL DO PRODUTO
   - Se houver imagem real, usa <img>
   - Se não houver, renderiza fallback visual por tipo:
     jar, pouch ou capsules
   ========================================================= */
...
/* =========================================================
   HELPERS DE PRODUTO
   - Funções utilitárias para recuperar e padronizar dados
   ========================================================= */
...
function getProductMetaTags(product) {
  return Array.isArray(product.metaTags) && product.metaTags.length
    ? product.metaTags
    : [];
}
...
function getProductBadges(product) {
  return Array.isArray(product.brandBadges) && product.brandBadges.length
    ? product.brandBadges
    : ['Produto específico', 'Verde Aura'];
}

/* =========================================================
   MODAL/PÁGINA DE DETALHE DO PRODUTO
   - Monta o conteúdo completo do produto selecionado
   - Exibe badges, descrição, benefícios, informações extras
   ========================================================= */
function openProductDetail(productId) {
  const product = getProductById(productId);

  if (!product || !elements.productDetailPage || !elements.productDetailContent) {
    return;
  }
  ...
  const visual = getProductVisual(product.type, product.image, product.name, {
    eager: true
  });
  ...
  elements.productDetailContent.innerHTML = `
    <div class="product-detail-visual-wrap">
      <div
        ...
        style="background: linear-gradient(180deg, ${product.color[0]}, ${product.color[1]});"
      >
        ${visual}
      </div>
    </div>
    ...
    <div class="product-detail-content">
      ...
      <div class="product-detail-actions">
        ...
        <span class="product-detail-mini">
          Ao clicar em “Adicionar ao carrinho”, o fluxo atual do site continua funcionando normalmente.
        </span>
      </div>
    </div>
  `;
...
/* =========================================================
   FECHAMENTO DA PÁGINA DE DETALHE
   ========================================================= */
...
/* =========================================================
   ...
   - Monta a grade do catálogo
   - Exibe estado vazio quando não encontrar resultados
   ========================================================= */
function renderProducts() {
  ...
  if (!filtered.length) {
    elements.productsGrid.innerHTML = `
      <article
        class="product-card"
        style="grid-column: 1 / -1; text-align:center; padding: 34px;"
      >
        <h3 style="margin-top:0;">Nenhum produto encontrado.</h3>
        <p class="product-description" style="margin-bottom:0;">
  ...
  elements.productsGrid.innerHTML = filtered
    .map((product, index) => `
      <article
        class="product-card product-tilt reveal visible"
        data-id="${product.id}"
        tabindex="0"
        role="button"
...
/* =========================================================
   CARRINHO
   - Adiciona, altera quantidade, remove item e renderiza
   ========================================================= */
...
/* =========================================================
   ...
   - Gera uma mensagem automática com os itens do carrinho
...
function generateWhatsAppMessage() {
  ...
  const lines = [
    ...
    '',
    ...state.cart.map((item) => `• ${item.name} — Qtd: ${item.quantity}`),
    '',
    'Gostaria de confirmar pagamento, entrega e disponibilidade.'
  ];
...
/* =========================================================
   OTIMIZAÇÃO DE EVENTOS COM REQUESTANIMATIONFRAME
   - Evita excesso de cálculos em eventos como mousemove
   ========================================================= */
...
/* =========================================================
   EFEITO TILT DOS CARDS
   - Aplica rotação 3D leve em cards com classe .product-tilt
...
/* =========================================================
   PARALLAX DO HERO
   - Move levemente os cards flutuantes conforme o mouse
   - Só ativa em telas maiores
   ========================================================= */
function setupHeroParallax() {
  if (!elements.heroStage) return;
  ...
  const handleMove = rafThrottle((event) => {
    ...
    const x = (event.clientX - centerX) / 22;
    const y = (event.clientY - centerY) / 22;

    document.querySelectorAll('.floating-card').forEach((card, index) => {
      const intensity = index === 0 ? 1 : 0.6;
      card.style.transform = `translate(${x * intensity}px, ${y * intensity}px)`;
    });
  });

  elements.heroStage.addEventListener('mousemove', handleMove, { passive: true });

  elements.heroStage.addEventListener('mouseleave', () => {
...
function bindEvents() {
  ...
  /* -----------------------------------------
     Clique na grade de produtos
     - Botão "Adicionar"
     - Abertura do detalhe do produto
     ----------------------------------------- */
  ...
  /* -----------------------------------------
     ...
     - Enter ou Espaço abre o detalhe
     ----------------------------------------- */
  ...
  /* -----------------------------------------
     Controles da página de detalhe
     ----------------------------------------- */
  ...
  /* -----------------------------------------
     Clique fora do conteúdo fecha detalhe
     ----------------------------------------- */
  elements.productDetailPage.addEventListener('click', (event) => {
    if (event.target === elements.productDetailPage) {
      closeProductDetail();
    }
  });
...
/* =========================================================
   ...
   - Ordem de carregamento da interface
   ========================================================= */
function init() {
  renderFilters();
  renderProducts();
